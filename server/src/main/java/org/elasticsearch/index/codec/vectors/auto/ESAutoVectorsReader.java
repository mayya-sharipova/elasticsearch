...
// Based on Lucene99HnswVectorsReader with 1 difference:
// - for search is graph is not present, we iterate over flat reader

public class ESAutoVectorsReader extends KnnVectorsReader implements QuantizedVectorsReader, HnswGraphProvider {
    private static final long SHALLOW_SIZE = RamUsageEstimator.shallowSizeOfInstance(ESAutoVectorsReader.class);
    private final FlatVectorsReader flatVectorsReader;
    private final FieldInfos fieldInfos;
    private final Map<String, FieldEntry> fields = new HashMap<>();
    private final IndexInput vectorIndex;

    public ESAutoVectorsReader(SegmentReadState state, FlatVectorsReader flatVectorsReader) throws IOException {
        ...
        try (ChecksumIndexInput meta = state.directory.openChecksumInput(metaFileName)) {
            ...
            try {
                versionMeta = CodecUtil.checkIndexHeader(
                    ...
                    ESAutoVectorFormat.VERSION_START,
    ...
    public long ramBytesUsed() {
        return ESAutoVectorsReader.SHALLOW_SIZE + RamUsageEstimator.sizeOfMap(
            fields,
    ...
    private void search(
        ...
        if (fieldEntry.size() == 0 || knnCollector.k() == 0 || fieldEntry.vectorEncoding != vectorEncoding) {
            return;
        }
        final RandomVectorScorer scorer = scorerSupplier.get();
        final KnnCollector collector = new OrdinalTranslatedKnnCollector(knnCollector, scorer::ordToDoc);
        final Bits acceptedOrds = scorer.getAcceptOrds(acceptDocs);
        // if graph exists and we need to k < number of vectors, use graph
        if (fieldEntry.vectorIndexLength > 0 && knnCollector.k() < scorer.maxOrd()) {
