...
import static org.elasticsearch.index.codec.vectors.auto.ESAutoVectorFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;

// Based on Lucene99HnswVectorsWriter with these differences:
// - for write: we use only flatVectorWriter until MAX_DOCS_IN_FLAT_SEGMENT, then we use hnsw writer
// - for merge the same: if total number of docs in a new merged segment < MAX_DOCS_IN_FLAT_SEGMENT we
//   use flat writer; otherwise hnsw writer

public final class ESAutoVectorsWriter extends KnnVectorsWriter {

    private static final int MAX_DOCS_IN_FLAT_SEGMENT = 250_000;
    private static final long SHALLOW_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(ESAutoVectorsWriter.class);
    ...
    public ESAutoVectorsWriter(
        ...
        String metaFileName = IndexFileNames.segmentFileName(
            ...
            state.segmentSuffix,
            ESAutoVectorFormat.META_EXTENSION
        );
        String indexDataFileName = IndexFileNames.segmentFileName(
            state.segmentInfo.name,
            state.segmentSuffix,
            ESAutoVectorFormat.VECTOR_INDEX_EXTENSION
        );
        ...
        try {
            ...
            CodecUtil.writeIndexHeader(
                meta,
                ESAutoVectorFormat.META_CODEC_NAME,
                ESAutoVectorFormat.VERSION_CURRENT,
                state.segmentInfo.getId(),
                state.segmentSuffix
            );
            CodecUtil.writeIndexHeader(
                vectorIndex,
                ESAutoVectorFormat.VECTOR_INDEX_CODEC_NAME,
                ESAutoVectorFormat.VERSION_CURRENT,
                state.segmentInfo.getId(),
                state.segmentSuffix
            );
    ...
    public void mergeOneField(FieldInfo fieldInfo, MergeState mergeState) throws IOException {
        ...
        try {
            ...
            if (scorerSupplier.totalVectorCount() >= MAX_DOCS_IN_FLAT_SEGMENT) {
                // Build graph only when there are enough docs in a new segment
    ...
    private static class FieldWriter<T> extends KnnFieldVectorsWriter<T> {
        ...
        public void addValue(int docID, T vectorValue) throws IOException {
            ...
            tentativeNode++;
            if (tentativeNode >= MAX_DOCS_IN_FLAT_SEGMENT) {
                // do graph; build all the nodes accumulated so far if necessary
                while (node < tentativeNode) {
