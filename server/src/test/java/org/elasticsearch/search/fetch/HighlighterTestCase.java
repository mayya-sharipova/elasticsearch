...
import org.elasticsearch.search.fetch.subphase.highlight.matchhighlight.MatchHighlighter;
...
public class HighlighterTestCase extends MapperServiceTestCase {
    ...
    protected final Map<String, HighlightField> highlight(MapperService mapperService, ParsedDocument doc, SearchSourceBuilder search)
        ...
        withLuceneIndex(mapperService, iw -> iw.addDocument(doc.rootDoc()), ir -> {
            ...
            FetchSubPhaseProcessor processor = highlightPhase.getProcessor(fetchContext(context, searcher, search));
            LeafReaderContext leafReaderContext = searcher.getTopReaderContext().leaves().get(0);
            FetchSubPhase.HitContext hitContext = new FetchSubPhase.HitContext(new SearchHit(0, "id", null, null), leafReaderContext, 0);
    ...
    private static FetchContext fetchContext(SearchExecutionContext context, ContextIndexSearcher searcher, SearchSourceBuilder search) throws IOException {
        ...
        when(fetchContext.searcher()).thenReturn(searcher);
        return fetchContext;
    }
}
